...
public class SonarFrameworkFunctions 
{
	...
	{	
		...
		if(SonarFrameworkSettings.USE_GOOGLE_PLAY_GAME_SERVICES)
		{
				...
				try {
					...
					googlePlayServices.SetActivity(app);
		...
		if(SonarFrameworkSettings.USE_GOOGLE_ANALYTICS)
		{
				...
				try {
					...
					googleAnalytics.SetActivity(app);
				} catch (InstantiationException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				} catch (IllegalAccessException e) {
		...
		if(SonarFrameworkSettings.USE_GOOGLE_PLAY_GAME_SERVICES)
		{
			app.runOnUiThread(new Runnable() 
			{
				...
				public void run() 
				{
					if (isSignedIn())
					{
						app.startActivityForResult(googlePlayServices.showAchievements(),REQUEST_ACHIEVEMENTS);
					}
					else 
					{
		...
		if(SonarFrameworkSettings.USE_GOOGLE_PLAY_GAME_SERVICES)
		{
			app.runOnUiThread(new Runnable() 
			{
				...
				public void run() 
				{
					if (isSignedIn())
					{
						app.startActivityForResult(googlePlayServices.showLeaderboards(),REQUEST_LEADERBOARDS);
					}
					else 
					{
		...
		if(SonarFrameworkSettings.USE_GOOGLE_PLAY_GAME_SERVICES)
		{
			app.runOnUiThread(new Runnable() 
			{
				...
				public void run() {
					if (isSignedIn())
					{
						app.startActivityForResult(googlePlayServices.showLeaderboard(leaderboardID),REQUEST_LEADERBOARD);
					}
					else 
					{
	...
	public static boolean isFullscreenAdReadyCB() {
		
		
		return ((sonar.systems.frameworks.ChartBoost.ChartBoostAds)chartboost).isFullscreenAdReady();
	}
