...
#elif(CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
#if SCH_IS_CHARTBOOST_ENABLED == true
    IOSCPPHelper::preloadChartboostFullScreenAd();
#endif
#endif
}
...
bool Chartboost::isFullscreenAdReady() {
#if(CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
    return JniHelpers::jniCommonBoolCall(
        "isFullscreenAdReadyCB",
        CLASS_NAME
    );
#elif(CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
#if SCH_IS_CHARTBOOST_ENABLED == true
    return IOSCPPHelper::isChartboostFullscreenAdReady();
#endif
#endif
return false;
}
